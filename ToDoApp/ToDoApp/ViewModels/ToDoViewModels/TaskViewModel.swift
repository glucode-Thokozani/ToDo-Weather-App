...
class TaskViewModel: ObservableObject {
    // Depend on abstractions for testability and SOLID compliance
    ...
    init(toDoRepository: ToDoRepositoryProtocol, categoryRepository: CategoryRepositoryProtocol) {
        self.toDoRepository = toDoRepository
        self.categoryRepository = categoryRepository
        self.categories = Array(categoryRepository.readCategories())
        observeCategories()
    }
    
    private func observeCategories() {
        let results = categoryRepository.readCategories()
    ...
    private func observeTodos() {
        ...
        let results = toDoRepository.readTodo(for: category)
    ...
    func toggleTaskCompletion(_ todo: ToDo) {
        do {
            try toDoRepository.toggleCompletion(for: todo)
        } catch {
    ...
    func deleteTask(_ todo: ToDo) {
        do{
            try toDoRepository.delete(todo: todo)
        } catch {
    ...
    func addCategory(name: String) {
        ...
        category.name = name
        do {
            try categoryRepository.addCategoryObject(category)
        } catch {
